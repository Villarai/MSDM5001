...
class CIM(QAIA):
    ...
    def __init__(
        ...
        self.a = a
        self.y = y
        self.spin_history = []  # 新增，保存每次迭代的自旋值
        self.initialize()
    ...
    def initialize(self):
        ...
        if self.x is None:
            ...
            self.spin_history.append(self.x.copy())  # 保存当前自旋值
        # gradient
...
if __name__ == "__main__":
    ...
    for node in range(s.N):  # 遍历每个节点
        plt.plot(range(s.n_iter+1), [spin[node] for spin in s.spin_history], linewidth = 0.5, label=f'Node {node + 1}')
        
    plt.title(f'Amplitudes Over Time')
    plt.xlabel('Time (iterations)')
    plt.ylabel('Amplitudes C_i(t)')
    plt.axhline(0, color='black', linewidth=0.5, ls='--')
    plt.grid()
    plt.legend(loc='upper right')  # Set a specific location for the legend
    plt.show()
